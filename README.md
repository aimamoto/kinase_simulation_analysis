...
Prompted by a concordance check against [Kincore-standalone](http://dunbrack.fccc.edu/kincore/) (Dunbrack lab) on PDB 3D7T. **v7r3 corrects errors identified in v7r2, now defunct; re-run any dataset rather than comparing across versions.** All three corrections are in the ChimeraX worker.
...
2. **`ActLoop_CT` anchor corrected.** v7r2 used HRD+6, which is not a conserved position (an Arg in CSK but an Ala in SRC). The C-terminal anchor is now the HRD arginine, HRD+1 — the anchor residue of Modi & Dunbrack's APE9-Arg contact. **Only the anchor changed.** The other end is inherited unchanged from v6r6: an all-atom minimum over the APE−6…APE−9 window (Kincore's APE7–APE10) at 5.5 Å, against Kincore's two named atoms (APE9 Cα ↔ Arg O) at 6.0 Å. `ActLoop_CT` is therefore an AlloQuant metric in its own right and is not comparable with Kincore's `ActLoopCT` label.
