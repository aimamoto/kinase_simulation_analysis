...
def main():
    ...
    if run_config:
        if not args.resume:
            ...
            if choice == "2":
                print("\n[*] PAUSED: Review 'generated_matrix.csv'.\n[*] When ready, run: python3 run_kinase_pipeline_v4.py --resume")
                sys.exit(0)
        else:
    ...
    run_step("1_run_parallel_chimerax_hmm_v4.py", ["-c", str(args.cores)], f"Parallel ChimeraX analysis ({args.cores} workers)")
