...
# --- CONFIGURATION ---
SEARCH_DIR = "."  
VIZ_OUT_DIR = "cx_pocket_visualization_hmm"
FILE_EXTENSION = ".cif"
...
# --- MASTER EXECUTION ---
def process_model(session, full_cif_path, base_dir, out_dir_viz, wt_dir, all_landmarks):
    csv_rows =[]
    rel_to_base = os.path.relpath(full_cif_path, base_dir)
    rel_to_out = os.path.relpath(full_cif_path, out_dir_viz).replace('\\', '/')
    sim_id = get_core_name(rel_to_base)
    
    lm = None
    for fasta_name, coords in all_landmarks.items():
        if fasta_name.lower() in sim_id.lower() or fasta_name.replace("_", "-").lower() in sim_id.lower():
            lm = coords; break
            
    if not lm:
        print(f"Skipping {sim_id}: No HMM landmarks found.")
        return csv_rows
    ...
    for cid in cids:
        ...
        if lm.get('f') is not None and lm['f'] < len(res):
            has_kinase = True
            ...
            if len(ligands) > 0:
                ...
                if anchor_pos is not None:
                    for lig_res in ligands:
                        if np.linalg.norm(anchor_pos - get_center_of_mass(lig_res.atoms)) < 25.0:
            ...
            if dfg_d_res is not None:
                d_oxygens = dfg_d_res.atoms[np.isin(dfg_d_res.atoms.names, ['OD1', 'OD2'])]
                ...
                if len(d_oxygens) > 0 and len(mg_atoms) > 0:
                    ...
                    dfg_mg_dist_val = round(float(np.min(np.linalg.norm(diff, axis=2))), 2)
            ...
            f_cz = get_atom(res[lm['f']], "CZ") if lm.get('f') is not None else None
            ...
            c_oe1 = get_atom(res[lm['c']], "OE1") if lm.get('c') is not None else None
            c_oe2 = get_atom(res[lm['c']], "OE2") if lm.get('c') is not None else None
            if k_nz is not None:
                dists =[float(np.linalg.norm(k_nz - oe)) for oe in[c_oe1, c_oe2] if oe is not None]
                if dists: sb_dist_val = round(min(dists), 2)

            # --- C-Helix Positional Anchor ---
            ...
            if idx_x - 1 >= 0 and idx_f + 1 < len(res):
                ...
                res_f = res[idx_f]
                chi1 = calculate_dihedral(get_atom(res_f, "N"), get_atom(res_f, "CA"), get_atom(res_f, "CB"), get_atom(res_f, "CG"))
            ...
            chain_data[cid] = {
                ...
                "meta": {"Type": lm['type'], "State": state, "CHelix": chelix_label,
                         ...
                         "ploop_res_nums": ploop_res_nums}
            }

    # =========================================================================
    # --- CHIMERAX VISUALIZATION & COLOR CODE LEGEND ---
    # =========================================================================
    # Chain Backbone      : Light Gray (#d3d3d3)
    # Activation Loop     : Dark Orange (N-term to DFG) & Coral (Dynamic Loop)
    # P-Loop Clamping     : Deep Sky Blue
    # R-Spine             : Medium Purple
    # Salt Bridge (K-C)   : Spring Green
    # Catalytic HRD-Asp   : Red
    # DFG-Asp Coordinate  : Dodger Blue
    # ATP / Ligands       : Gold Carbons
    # Magnesium (MG)      : Green
    # WT Reference Overlay: Dim Gray (70% Transparency)
    # =========================================================================
    
    # 1. Extract the specific protein name from the HMM landmarks file
    protein_name = "Unknown Kinase"
    for fasta_name in all_landmarks.keys():
        if fasta_name.lower() in sim_id.lower() or fasta_name.replace("_", "-").lower() in sim_id.lower():
            protein_name = fasta_name
            break

    cxc_lines =[
        f"# KinCore Visualization Macro for {sim_id}",
        f"open {rel_to_out}",
        "graphics silhouettes true", "graphics silhouettes width 1.5",
        "lighting soft", "color white", "hide atoms", "show cartoons",
        "transparency 50 cartoons", "style stick", "2dlabels delete all"
    ]

    # 2. Add the Top-Left Title and Top-Right Legend
    cxc_lines.extend([
        "\n# --- On-Screen Titles & Legend ---",
        # Top Left: Protein Name Title
        f"2dlabels create title_prot text '{protein_name}' color white size 24 bold true xpos 0.05 ypos 0.92",
        ...
        "2dlabels create leg_lig text 'ATP / Magnesium' color gold size 16 xpos 0.80 ypos 0.68",
        "2dlabels create leg_wt text 'Wild-Type Overlay' color dimgray size 16 xpos 0.80 ypos 0.65"
    ])

    if len(ligands) > 0:
        cxc_lines.extend([
            "\n# --- Ligands & Interactions ---",
            "show :ATP,ADP,ANP,ACP,AGS,AMP,GTP,GDP",
            "color :ATP,ADP,ANP,ACP,AGS,AMP,GTP,GDP byhetero",
            "color :ATP,ADP,ANP,ACP,AGS,AMP,GTP,GDP@C* gold",
            ...
            "contacts :ATP,ADP,ANP,ACP,AGS,AMP,GTP,GDP restrict protein",
            "show :MG", 
            "color :MG green", 
            "style :MG sphere", 
            "size :MG atomRadius 1.0"
        ])

    # The State Labels go on the Left side, starting right below the Protein Name
    y_offset = 0.88
    f_num_focus = None
    ...
    for cid, data in chain_data.items():
        ...
        csv_rows.append({
            ...
            "Spatial": meta['Spatial'], "Dihedral": meta['Dihedral'],
        ...
        if data['landmarks']:
            ...
            if f_num_focus is None: f_num_focus = f_num
            spec = f"/{cid}"
            
            cxc_lines.extend([
                f"\n# --- Chain {cid} ---",
                # Note: Font size is now 20
                f"2dlabels create State_{cid} text 'Chain {cid}: {meta['State']} (NT {meta['ActLoop_NT']}/CT {meta['ActLoop_CT']})' color white size 20 xpos 0.05 ypos {y_offset}",
                f"color {spec} #d3d3d3",
                f"show {spec}:{d_num}-{f_num+1}",
                f"color {spec}:{d_num}-{f_num+1} dark orange"
            ])

            # P-Loop
            if meta.get('ploop_res_nums'):
                p_start, p_end = meta['ploop_res_nums'][0], meta['ploop_res_nums'][-1]
                cxc_lines.extend([
                    f"\n# P-Loop Clamping", 
                    f"color {spec}:{p_start}-{p_end} deep sky blue"
                ])

            # A-Loop
            if lm.get('ape') is not None:
                ape_num = res[lm['ape']].number
                cxc_lines.extend([
                    f"# Dynamic A-Loop Visualization",
                    f"color {spec}:{f_num+1}-{ape_num} coral",
                    f"show {spec}:{ape_num} & sidechain"
                ])
                
            # R-Spine
            if all(lm.get(x) is not None for x in['hrd', 'f', 'rs1', 'rs2']):
                r1 = res[lm['rs2']].number; r2 = res[lm['rs1']].number
                r3 = res[lm['f']].number; r4 = res[lm['hrd']].number
                cxc_lines.extend([
                    f"\n# R-Spine for Chain {cid}",
                    f"color {spec}:{r1},{r2},{r3},{r4} medium purple",
                    f"show {spec}:{r1},{r2},{r3},{r4} & sidechain"
                ])
                
            # Salt Bridge
            if lm.get('k') is not None and lm.get('c') is not None:
                k_num = res[lm['k']].number
                c_num = res[lm['c']].number
                cxc_lines.extend([
                    f"\n# Salt Bridge for Chain {cid}",
                    f"color {spec}:{k_num},{c_num} spring green",
                    f"show {spec}:{k_num},{c_num} & sidechain",
                    f"distance {spec}:{k_num}@NZ {spec}:{c_num}@OE1 color spring green radius 0.05",
                    f"distance {spec}:{k_num}@NZ {spec}:{c_num}@OE2 color spring green radius 0.05"
                ])
            ...
            if hrd_d_num:
                cxc_lines.extend([
                    f"\n# Catalytic HRD-Asp",
                    f"show {spec}:{hrd_d_num} & sidechain",
                    f"color {spec}:{hrd_d_num} & sidechain red"
                ])
                if meta.get('atp_spec') and meta.get('closest_d_o_name'):
                    d_o_tgt = f"{spec}:{hrd_d_num}@{meta['closest_d_o_name']}"
                    cxc_lines.append(f"distance {d_o_tgt} {meta['atp_spec']} color magenta radius 0.05")

            # DFG-Asp
            if meta.get('dfg_d_res'):
                dfg_d_num = meta['dfg_d_res'].number
                cxc_lines.extend([
                    f"\n# DFG-Asp Coordination Base",
                    f"show {spec}:{dfg_d_num} & sidechain",
                    f"color {spec}:{dfg_d_num} & sidechain dodger blue"
                ])
                if meta.get('DFG_Mg_Dist') != "N/A":
                    cxc_lines.append(f"distance {spec}:{dfg_d_num}@OD1,OD2 :MG color green radius 0.05")
            
            # Decrease y_offset by 0.04 for the next chain (tighter spacing for size 20 font)
            y_offset -= 0.04

    if has_kinase and f_num_focus:
        cxc_lines.extend(["\n# Final Polish", "hide H", f"view /{cids[0]}:{f_num_focus}", "zoom 1.5", "clip off"])
        
        # RESTORED WILD TYPE OVERLAY
        matched_wt = find_wt_reference(sim_id, wt_dir)
        if matched_wt:
            cxc_lines.extend([
                "\n# =========================================================",
                "# --- AUTOMATED WILD-TYPE OVERLAY MODULE ---",
                f"open '{matched_wt}'",
                "mmaker #2 to #1",
                "transparency 70 #2 cartoons",
                "color #2 dimgray",
                "hide #2 atoms",
                "# ========================================================="
            ])

        with open(os.path.join(out_dir_viz, f"viz_{sim_id}.cxc"), 'w') as out_f:
            out_f.write("\n".join(cxc_lines))
...
def main(session):
    ...
    out_dir_viz = os.path.join(base_dir, VIZ_OUT_DIR)
    os.makedirs(out_dir_viz, exist_ok=True)
    
    chunk_list_file = os.environ.get("CHIMERAX_CHUNK")
    if not chunk_list_file or not os.path.exists(chunk_list_file):
        print(f"Worker Error: Missing CHIMERAX_CHUNK."); run(session, "quit"); return
    ...
    cols =["Simulation_ID", "Directory", "File", "Chain", "Type", "State", 
            "R_Spine", "C_Spine", "C_Helix", "Spatial", "Dihedral", "ActLoop_NT", "ActLoop_CT", 
            "Phi_D", "Psi_D", "D1_Dist", "D2_Dist", "SB_Dist", 
            "HRD_ATP_Dist", "DFG_Mg_Dist", "DFG_ATP_Dist", "PLoop_ATP_Dist"]
           
    # Fetch the Wild-Type directory from the environment
    wt_dir = os.environ.get("CHIMERAX_WT_DIR")

    # Using list() instead of empty brackets to bypass the truncation bug!
    all_rows = list() 
    
    for full_cif_path in files_to_process:
        try:
            rows = process_model(session, full_cif_path, base_dir, out_dir_viz, wt_dir, all_landmarks)
            all_rows.extend(rows)
        except Exception as e:
            print(f"Error processing {full_cif_path}: {e}")

    # Save to the worker-specific CSV
