...
def process_model(session, full_cif_path, base_dir, out_dir_viz, wt_dir, all_landmarks, fasta_seqs):
    ...
    for cid in cids:
        ...
        if lm.get('hrd') is not None and lm['hrd'] + 2 < len(res) and chain_ligand_atoms is not None:
            ...
            # Dynamically grab polar sidechain atoms to support atypical bases (e.g., Asn in ERBB3)
        ...
        # DFG-Asp & Mg2+ Coordination (Updated for Atypical Motifs & Exact Mg Targeting)
        ...
        if dfg_d_res is not None:
            ...
            if len(d_oxygens) > 0 and len(mg_atoms) > 0:
                ...
                # Robustly target the exact Mg atom to prevent ChimeraX selection errors
        ...
        # Fallback for atypical DFG motifs (e.g., DLG mutation lacking CZ)
        if f_cz is None and f_res is not None:
            ...
            if f_sc is not None and f_ca is not None and len(f_sc) > 0:
                ...
                f_cz = f_sc.scene_coords[np.argmax(dists)] # Map to furthest sidechain heavy atom
        ...
        # Robustly track sidechain polar atoms to support atypical kinases (e.g., K-H interaction in ERBB3)
        sb_oe_names =[]
        if k_nz is not None and c_res is not None:
            ...
            if len(c_polar_atoms) > 0:
                ...
                # Keep up to 2 closest polar atoms for visualization to avoid clutter
    ...
    for cid, data in chain_data.items():
        ...
        if data['landmarks']:
            ...
            if lm.get('k') is not None and lm.get('c') is not None:
                ...
                # Dynamic generation prevents "No atoms match" macro crashes!
            ...
            if meta.get('dfg_d_res'):
                ...
                if meta.get('DFG_Mg_Dist') != "N/A" and meta.get('closest_dfg_mg_atom') and meta.get('closest_mg_spec'):
                    # Use exact dynamically generated atom specs to prevent multi-atom or 0-atom selection crashes
    ...
    if has_kinase:
        ...
        # Dynamically frame *ALL* analyzed active sites so heterodimers are fully visible
