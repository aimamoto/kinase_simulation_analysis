...
def get_base_sim_name(name):
    """Strips AlphaFold3/ColabFold suffixes to find the true core protein name."""
...
# --- MASTER EXECUTION ---
def process_model(session, full_cif_path, base_dir, out_dir_viz, wt_dir, all_landmarks, fasta_seqs):
    csv_rows =[]
    ...
    # Strip suffixes to make a robust match base
    ...
    for fasta_name, coords in all_landmarks.items():
        ...
        # Flexible matching allows 'seed-1/model.cif' to map perfectly to HMM landmarks extracted from the top-rank FASTA
    ...
    # Deduplicate in case multiple conditions match
    unique_lms = {}
    ...
    if not candidate_lms: 
        print(f"  [?] Skipping {sim_id}: Could not match any HMM landmarks.")
        return csv_rows
    ...
    for cid in cids:
        ...
        if len(ligands) > 0:
            ...
            if anchor_pos is not None:
                for lig_res in ligands:
                    # Reduced from 25.0 to 12.0 to prevent reaching across the dimer interface
                    if np.linalg.norm(anchor_pos - get_center_of_mass(lig_res.atoms)) < 12.0:
        ...
        # Catalytic Clamping (Updated for Pseudokinases)
        if lm.get('hrd') is not None and lm['hrd'] + 2 < len(res) and chain_ligand_atoms is not None:
            hrd_d_res = res[lm['hrd'] + 2]
            
            # Dynamically grab polar sidechain atoms to support atypical bases
        ...
        # Spatial Anchors & Salt Bridge Validation
    ...
    cxc_lines.extend([
        ...
        "2dlabels create leg_wt text 'Wild-Type Overlay' color dimgray size 16 xpos 0.80 ypos 0.59"
    ])
    ...
    for cid, data in chain_data.items():
        ...
        if data['landmarks']:
            ...
            cxc_lines.extend([
                f"\n# --- Chain {cid} ---",
                f"2dlabels create State_{cid} text 'Chain {cid} ({meta['Type']}): {meta['State']} (NT {meta['ActLoop_NT']}/CT {meta['ActLoop_CT']})' color white size 20 xpos 0.05 ypos {y_offset}",
                f"color {spec} #d3d3d3",
            ...
            if all(lm.get(x) is not None for x in['hrd', 'f', 'rs1', 'rs2']):
                r1 = res[lm['rs2']].number
                r2 = res[lm['rs1']].number
                r3 = res[lm['f']].number
                r4 = res[lm['hrd']].number
                cxc_lines.extend([
                    f"\n# R-Spine for Chain {cid}",
                    f"color {spec}:{r1},{r2},{r3},{r4} medium purple",
                    f"show {spec}:{r1},{r2},{r3},{r4} & sidechain",
                    # Draw the 3 connecting segments of the R-Spine
            ...
            if lm.get('k') is not None and lm.get('c') is not None:
                ...
                for oe_name in meta.get('sb_oe_names',[]):
                    cxc_lines.append(f"distance {spec}:{k_num}@NZ {spec}:{c_num}@{oe_name} color spring green radius 0.05")

            # --- NEW in v3: Hydrophobic Shell & Bridges ---
            # --- NEW: Hydrophobic Shell & Bridges ---
            if all(lm.get(x) is not None for x in ['v104', 'm118', 'm120', 'rs2']):
                v104_num = res[lm['v104']].number
                m118_num = res[lm['m118']].number
                m120_num = res[lm['m120']].number
                rs2_num = res[lm['rs2']].number
                cxc_lines.extend([
                    f"\n# Hydrophobic Shell & V104 Bridge for Chain {cid}",
                    f"color {spec}:{v104_num},{m118_num},{m120_num} teal",
                    f"show {spec}:{v104_num},{m118_num},{m120_num} & sidechain",
                    # 1. Visualize Shell Cohesiveness (M118 to M120)
                    f"distance {spec}:{m118_num}@CA {spec}:{m120_num}@CA color teal radius 0.05",
                    # 2. Visualize V104 Bridge to R-Spine (RS2)
                    f"distance {spec}:{v104_num}@CA {spec}:{rs2_num}@CA color teal radius 0.05"
                ])
                
            if lm.get('i150') is not None and lm.get('hrd') is not None:
                i150_num = res[lm['i150']].number
                hrd_num = res[lm['hrd']].number
                cxc_lines.extend([
                    f"\n# I150 aE Bridge to HRD for Chain {cid}",
                    f"color {spec}:{i150_num} tan",
                    f"show {spec}:{i150_num} & sidechain",
                    # 3. Visualize I150 Bridge to Catalytic HRD His
                    f"distance {spec}:{i150_num}@CA {spec}:{hrd_num}@CA color tan radius 0.05"
                ])
            # ----------------------------------------
            ...
            if hrd_d_num:
                ...
                if meta.get('atp_spec') and meta.get('closest_d_o_name'):
                    # Prevent cross-dimer rendering by capping the visual line at 10 Angstroms
                    if meta.get('HRD_ATP_Dist') != "N/A" and meta['HRD_ATP_Dist'] < 10.0:
            ...
            if meta.get('dfg_d_res'):
                ...
                if meta.get('DFG_Mg_Dist') != "N/A" and meta.get('closest_dfg_mg_atom') and meta.get('closest_mg_spec'):
                    # Prevent cross-dimer rendering by capping the visual line at 10 Angstroms
                    if meta['DFG_Mg_Dist'] < 10.0:
                        cxc_lines.append(f"distance {spec}:{dfg_d_num}@{meta['closest_dfg_mg_atom']} {meta['closest_mg_spec']} color green radius 0.05")
    ...
    if has_kinase:
        ...
        if view_targets:
            cxc_lines.extend([f"view {view_targets}", "zoom 1.2", "clip off"])
...
def main(session):
    ...
    if os.path.exists(fasta_path):
        with open(fasta_path, 'r') as f:
            ...
            for line in f:
                ...
                if line.startswith(">"):
                    if name:
                        fasta_seqs[name] = "".join(seq_lines)
    ...
    for full_cif_path in files_to_process:
        try:
            rows = process_model(session, full_cif_path, base_dir, out_dir_viz, wt_dir, all_landmarks, fasta_seqs)
