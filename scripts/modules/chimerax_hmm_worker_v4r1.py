...
def get_best_landmark_for_chain(chain_seq, cid, candidate_lms, fasta_seqs):
    best_lm = None
    best_name = "Unknown"
    best_score = -1.0
    used_ratio = False
    ...
    for fasta_name, coords in candidate_lms:
        # 1. Structural Motif Score (Max 10 points)
        struct_score = 0
        ...
        # 2. Sequence Identity Score (Comparing 3D Structure to 1D FASTA)
        ratio = 0.0
        ref_seq = None
        for header, fseq in fasta_seqs.items():
            if fasta_name.lower() in header.lower() or header.lower() in fasta_name.lower():
                ref_seq = fseq
                break
        
        if ref_seq:
            # autojunk=False prevents Python from ignoring common amino acids!
        ...
        # 3. Composite Score (Max 110 points)
        # Sequence Identity overwhelmingly dominates.
        composite = (ratio * 100) + struct_score
        
        # Explicit Chain ID Tag Bonus (If the FASTA header targets this exact chain)
    ...
    if used_ratio:
        # If fasta was matched, require at least a score of 15 (e.g., 15% seq identity)
        if best_score >= 15.0:
            return best_name, best_lm
    else:
        # If no fasta provided, fallback to requiring 3 points of pure structural motif
        if best_score >= 3.0:
            return best_name, best_lm
...
def process_model(session, full_cif_path, base_dir, out_dir_core, out_dir_allo, wt_dir, all_landmarks, fasta_seqs):
    ...
    for cid in cids:
        ...
        # ---> UPDATED: Added `cid` as the second argument <---
        lm_name, lm = get_best_landmark_for_chain(seq, cid, candidate_lms, fasta_seqs)
        
        if lm is None or lm.get('f') is None or lm['f'] >= len(res): continue
            
        # We REMOVED the pool deletion line here so homodimers work correctly!
