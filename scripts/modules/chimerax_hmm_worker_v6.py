...
def clean_protein_type(raw_type: str) -> str:
    """Strips construct modifiers (cat, cattail, tail, wt) to return pure base name."""
    name = re.sub(r'^\d+[_\-|]', '', raw_type)
    clean = re.sub(r'(wt|cattail|cat|tail)+$', '', name, flags=re.IGNORECASE)
    return clean.upper() if clean else raw_type.upper()
...
def get_best_landmark_for_chain(chain_seq: str, cid: str, candidate_lms: List, fasta_seqs: Dict) -> Tuple[str, Optional[Dict]]:
...
def process_model(session, full_cif_path: str, base_dir: str, out_dir_core: str, out_dir_allo: str, all_landmarks: Dict, fasta_seqs: Dict) -> List[Dict]:
    ...
    for cid in cids:
        ...
        # Strictly output only the Base Protein (e.g. EGFR) to the 'Type' column!
        clean_type_string = clean_protein_type(lm_name.split('-')[0])
    ...
    for i in range(len(cids_valid)):
        for j in range(i+1, len(cids_valid)):
            ...
            if chain_data[cA]['landmarks'] and chain_data[cB]['landmarks']:
                ...
                if role_a != "Unpaired":
                    chain_roles[cA] = role_a; chain_roles[cB] = role_b
                    partner_map[cA] = cB; partner_map[cB] = cA
                    int_dist_AC_BN[cA] = dAC_BN; int_dist_AC_BN[cB] = dBC_AN 
                    int_dist_BC_AN[cA] = dBC_AN; int_dist_BC_AN[cB] = dAC_BN
