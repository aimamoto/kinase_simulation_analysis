...
df <- map_dfr(all_csvs, ~read_csv(.x, col_types = cols(.default = "c"), na = c("", "NA", "N/A", "None"), show_col_types = FALSE)) %>%
...
if(nrow(candidate_sims) > 0 && length(unique(candidate_sims$Condition_designed)) > 1) {
  ...
  pref_summary <- candidate_sims %>%
    ...
    summarise(Total_Simulations = n(),
              ...
              Mismatch_Rate_Pct = (Ligand_Swapped / Total_Simulations) * 100)
...
if (CLUSTER_METHOD == "kmeans") {
  ...
  # --- RESTORED: Output the Gap Statistic Plot for K-Means Proof! ---
  suppressWarnings({
