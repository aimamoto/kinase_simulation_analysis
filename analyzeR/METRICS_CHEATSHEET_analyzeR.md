...
* **Phase 6: Unsupervised 3D Discovery** – PCA and clustering (GMM/K-Means) to identify meta-stable states. Outputs interactive HTML 3D scatter plots and static convex hull mappings.
...
Inside, you will find high-resolution `.pdf` graphics structured by Phase, rigorous `.csv` statistical readouts (Wilcoxon, Fisher, Kruskal-Wallis), and the critical `Phase7` metadata file required for downstream Phase 8 analyses.
